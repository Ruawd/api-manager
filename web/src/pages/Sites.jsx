...
export default function Sites() {
  ...
  // 切换API密钥可见性
  const toggleApiKeyVisibility = useCallback((siteId) => {
  ...
  const columns = [
    ...
    {
      title: <span style={{ fontSize: 15, fontWeight: 600 }}>API密钥</span>,
      dataIndex: 'apiKey',
      width: 280,
      render: (apiKey, record) => {
        const isVisible = visibleApiKeys.has(record.id)
        const displayKey = isVisible ? apiKey : apiKey ? `${apiKey.slice(0, 8)}...${apiKey.slice(-4)}` : '-'
        return (
          <Space size={4}>
            <Typography.Text
              code
    ...
    >
      ...
      {searchKeyword ? (
        <>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(360px, 400px))',
            gap: 12
          }}>
            {list.map(site => (
              <MobileSiteCard
